    ...
    const formatMessageText = (text: string) => {
        return text.split('\n').map((line, idx) => {
            ...
            if (headerMatch) {
                ...
                return (
                    <p key={idx} dangerouslySetInnerHTML={{ __html: `<strong>${cleanHeader}</strong>` }} style={{ margin: '12px 0 6px 0', fontSize: '1.05rem', color: 'var(--text-main)' }} />
                );
            }
            ...
            if (listMatch) {
                ...
                return (
                    <li key={idx} dangerouslySetInnerHTML={{ __html: cleanText }} style={{ marginLeft: '20px', listStyleType: 'disc' }} />
                );
            }
 
            return (
                <p key={idx} dangerouslySetInnerHTML={{ __html: processedLine }} style={{ margin: '4px 0' }} />
            );
        });
    };
