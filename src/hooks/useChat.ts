...
export default function useChat(currentUserId: string) {
  ...
  useEffect(() => {
    ...
    const handleReceiveMessage = (msg: Message) => {
      ...
      const msgGroupId = msg.grupo ? (typeof msg.grupo === 'string' ? msg.grupo : (msg.grupo as any)._id) : null;
    ...
    const handleMessageSent = (msg: Message) => {
      if (activeContact && !activeContact.isGroup) {
        const destId = msg.destinatario ? (typeof msg.destinatario === 'string' ? msg.destinatario : msg.destinatario._id) : '';
    ...
    const handleTyping = ({ userId, userName, grupoId }: { userId: string; userName?: string; grupoId?: string }) => {
      if (activeContact) {
        if (activeContact.isGroup) {
    ...
    const handleMessagesDeleted = ({ messageIds, type, grupoId }: { messageIds: string[]; type: 'me' | 'everyone'; grupoId?: string }) => {
      if (activeContact) {
  ...
  const emitStopTyping = useCallback(() => {
    if (!activeContact || !socket) return;
    socket.emit('stop_typing', { destinatarioId: activeContact._id, isGroup: activeContact.isGroup });
  }, [activeContact, socket]);
